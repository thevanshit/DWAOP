...
export default function AdvancedOverview({ subjects, announcements, upcomingClasses, timetable, isCR = false }: AdvancedOverviewProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-white via-gray-50/50 to-white rounded-2xl border border-gray-200/50 p-6 shadow-sm">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          {/* Left: Status Engine */}
          <div className="flex flex-col md:flex-row md:items-center gap-6">
            {/* Academic Status */}
            <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 ${eligibility.border} bg-white/50`}>
              ...
              <span className="text-sm text-gray-500">Academic Status</span>
            </div>
            ...
            {/* Next Action */}
            <div className="flex items-center gap-3 px-4 py-2.5 bg-blue-50/50 rounded-xl border border-blue-200/30">
              <Zap className="w-5 h-5 text-blue-600" />
              <div>
                <span className="text-xs text-blue-600 font-medium">Next Action: </span>
                <span className="text-sm font-semibold text-gray-900">{nextAction.action}</span>
                <span className="text-xs text-gray-500"> ({nextAction.time})</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="relative w-20 h-20">
              <svg className="w-20 h-20 transform -rotate-90">
                <circle cx="40" cy="40" r="35" stroke="currentColor" strokeWidth="6" fill="none" className="text-gray-100" />
                <circle 
                  cx="40" cy="40" r="35" 
                  stroke="currentColor" 
                  strokeWidth="6" 
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-xl font-bold text-gray-900">{overallAttendance}%</span>
              </div>
            </div>
            ...
            <div className="hidden md:block">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-sm text-gray-500">Semester Progress</span>
                <span className="text-sm font-bold text-gray-900">{semesterProgress}%</span>
              </div>
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="flex items-center gap-2 px-4 py-3 bg-white rounded-xl border border-gray-100 shadow-sm overflow-x-auto">
            <div className="flex items-center gap-2 shrink-0">
              <span className={`w-2 h-2 rounded-full ${overallAttendance >= 75 ? 'bg-green-500' : 'bg-amber-500 animate-pulse'}`} />
              <span className="text-sm text-gray-600">Attendance OK</span>
            </div>
            <span className="text-gray-200">|</span>
            <div className="flex items-center gap-2 shrink-0">
              <span className={`w-2 h-2 rounded-full ${riskSubjects.length > 0 ? 'bg-red-500 animate-pulse' : 'bg-green-500'}`} />
              <span className="text-sm text-gray-600">{riskSubjects.length} subjects risky</span>
            </div>
            <span className="text-gray-200">|</span>
            <div className="flex items-center gap-2 shrink-0">
              <span className={`w-2 h-2 rounded-full ${pendingAssignments > 0 ? 'bg-amber-500' : 'bg-green-500'}`} />
              <span className="text-sm text-gray-600">{pendingAssignments} pending deadlines</span>
            </div>
            <span className="text-gray-200">|</span>
            <div className="flex items-center gap-2 shrink-0">
              <span className={`w-2 h-2 rounded-full ${overallAttendance >= 75 ? 'bg-green-500' : 'bg-red-500 animate-pulse'}`} />
              <span className="text-sm text-gray-600">Eligibility {overallAttendance >= 75 ? 'Safe' : 'At Risk'}</span>
            </div>
          </div>

          {/* ZONE 3: SUBJECT CONTROL PANEL - Horizontal Modules */}
          <div>
            <h3 className="text-lg font-bold text-gray-900 mb-4">Subject Control Center</h3>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
            <h3 className="text-sm font-semibold text-gray-500 mb-4">Academic Flow</h3>
                  ...
                  <div className="flex flex-col items-center">
                    <div className={`
                      ...
                      ${stage.status === 'completed' ? 'bg-green-500 text-white' : 
                        stage.status === 'current' ? 'bg-blue-500 text-white ring-4 ring-blue-100' : 
                    ...
                    <span className="text-xs font-medium mt-2 text-gray-600">{stage.label}</span>
                  </div>
                  {idx < flowStages.length - 1 && (
                    <div className={`w-16 md:w-24 h-0.5 mx-2 ${stage.status === 'completed' ? 'bg-green-500' : 'bg-gray-200'}`} />
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
            <h3 className="text-sm font-semibold text-gray-500 mb-4">Smart Timeline</h3>
                  ...
                  <div className="flex-1 space-y-2">
                    {day.classes.slice(0, 2).map((cls, cidx) => (
                      <div key={cidx} className={`
                        ...
                        ${day.day === 'Today' ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-100'}
                      `}>
                        <div className="flex items-center gap-3">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
            <h3 className="text-sm font-semibold text-gray-500 mb-4">Upcoming</h3>
            <div className="space-y-3">
              <div className="p-3 bg-amber-50 rounded-xl border border-amber-100">
                ...
                <p className="text-sm font-bold text-gray-900">March 1-5</p>
              </div>
              <div className="p-3 bg-blue-50 rounded-xl border border-blue-100">
                ...
                <p className="text-sm font-bold text-gray-900">DBMS - Feb 18</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <div className={`p-4 rounded-xl border ${status.border} ${status.bg} transition-all hover:shadow-md`}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <h4 className="text-base font-bold text-gray-900">{subject.name}</h4>
          <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${status.bg} ${status.text}`}>
            {status.label}
          </span>
        </div>
        <span className="text-xl font-bold text-gray-900">{subject.attendance}%</span>
      </div>
      ...
      <div className="flex items-center justify-between">
        {classesNeeded > 0 ? (
          <p className="text-sm text-gray-600">
            Attend <span className="font-bold text-gray-900">{classesNeeded} more</span> classes to reach safe zone
          </p>
        ) : (
