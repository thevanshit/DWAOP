...
export default function ProfilePage() {
    ...
    const academicStats = [
        { label: 'Current CGPA', value: studentData.currentCGPA.toString(), icon: <Award className="w-5 h-5" />, color: 'bg-green-50 text-green-600', border: 'border-green-200' },
        { label: 'Attendance', value: `${studentData.overallAttendance}%`, icon: <Clock className="w-5 h-5" />, color: 'bg-blue-50 text-blue-600', border: 'border-blue-200' },
        { label: 'Semester', value: `Sem ${studentData.semester}`, icon: <BookOpen className="w-5 h-5" />, color: 'bg-purple-50 text-purple-600', border: 'border-purple-200' },
        { label: 'Credits Earned', value: '96', icon: <GraduationCap className="w-5 h-5" />, color: 'bg-amber-50 text-amber-600', border: 'border-amber-200' },
    ]
    ...
    return (
        <DashboardLayout role={userRole} roleLabel="Student" navItems={navItems}>
            <div className="mb-8">
                ...
                <p className="text-base text-gray-600 mt-1">View and manage your personal information</p>
            </div>

            {/* Hero Section */}
            <div className="bg-gradient-to-br from-white via-gray-50 to-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-6">
                <div className="h-32 bg-gradient-to-r from-[var(--color-primary)] via-blue-500 to-indigo-600"></div>
                <div className="px-6 pb-6">
                    <div className="flex flex-col md:flex-row md:items-end gap-6 -mt-12">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-gray-600 font-medium">{studentData.rollNumber}</p>
                        ...
                        <div className="flex gap-3">
                            <button className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-200 hover:bg-gray-50 rounded-xl transition-colors shadow-sm">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                {academicStats.map((stat, idx) => (
                    <div key={idx} className={`bg-white rounded-xl border ${stat.border} p-5 shadow-sm hover:shadow-md transition-shadow`}>
            ...
            <div className="grid lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                    <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
                        ...
                        <h3 className="font-semibold text-gray-900">Performance & Enrollment</h3>
                    </div>
                    <div className="p-6 space-y-4">
    ...
    return (
        <div className="flex items-start justify-between py-2 border-b border-gray-50 last:border-0">
            <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${highlight ? 'bg-blue-50 text-blue-600' : 'bg-gray-50 text-gray-400'}`}>
                    {icon}
                </div>
                <span className="text-sm text-gray-600">{label}</span>
            </div>
            <span className={`text-sm font-semibold ${highlight ? 'text-blue-600' : 'text-gray-900'} text-right max-w-[60%] truncate`}>{value}</span>
        </div>
    )
}
