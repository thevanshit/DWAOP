...
import { useTeacherDashboardContext, type TimetableEntry } from './TeacherDashboardProvider'
...
export function TimetableView() {
  ...
  // Fix Sunday off-by-one: getDay() returns 0 for Sunday, 1 for Monday, etc.
  const dayIndex = new Date().getDay()
  const currentDay = dayIndex === 0 ? 'Sun' : DAY_NAMES[dayIndex - 1] || 'Mon'

  const getDayClasses = (day: string): TimetableEntry[] => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {DAY_NAMES.slice(0, 5).map((day, idx) => {
          ...
          const classes = getDayClasses(dayKey)
          return (
            <motion.div 
              key={day} 
